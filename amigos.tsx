...
type ProfileRow = { id: string; username: string; friend_code: string; points: number };
  ...
  return useQuery({
    ...
    queryFn: async () => {
      ...
      return (data ?? []).map((f) => ({
        ...
        other: byId.get(f.requester_id === myId ? f.addressee_id : f.requester_id),
      }));
    },
  });
}
...
function FriendsPage() {
  ...
  async function search() {
    ...
    if (!q || !me) return;
    setSearching(true);
    const term = q.toUpperCase().startsWith("CX-") ? q.toUpperCase() : q.toLowerCase();
    const { data, error } = await supabase.rpc("search_profiles", { _q: term });
    setSearching(false);
    if (error) {
  ...
  async function respond(id: string, accept: boolean) {
    ...
    await queryClient.invalidateQueries({ queryKey: ["friendships"] });
    toast.success(accept ? "Ya sois amigos" : "Solicitud rechazada");
  }
    ...
    <AppShell subtitle="Amigos">
      <div className="space-y-10">
        <section className="journal-in border border-foreground p-5">
          ...
          <p className="mt-2 font-mono text-2xl tracking-[0.2em]">{me?.friend_code ?? "—"}</p>
          <button
            type="button"
            onClick={() => {
              if (me?.friend_code) {
        ...
        <section className="journal-in">
          <h2 className="mb-3 font-display text-xl font-bold italic">Buscar coleccionistas</h2>
          ...
          <div className="mt-4 divide-y divide-border">
            {results.map((row) => (
              <div key={row.id} className="flex items-center justify-between py-3">
                <span>
                  <span className="block font-display text-base">{row.username}</span>
                  <span className="font-mono text-[9px] uppercase text-muted-foreground">
                    {row.friend_code} · {formatPoints(row.points)} pts
                  </span>
                </span>
                <button
                  type="button"
        ...
        {pendingIncoming.length > 0 && (
          <section className="journal-in">
            <h2 className="mb-3 font-display text-xl font-bold italic">Solicitudes recibidas</h2>
            <div className="divide-y divide-border">
              {pendingIncoming.map((l) => (
                <div key={l.id} className="flex items-center justify-between py-3">
                  <span className="font-display text-base">{l.other?.username ?? "—"}</span>
        ...
        <section className="journal-in">
          ...
          <div className="divide-y divide-border">
            {accepted.map((l) => (
              <div key={l.id} className="flex items-center justify-between py-3">
                <span>
                  <span className="block font-display text-base">{l.other?.username ?? "—"}</span>
                  <span className="font-mono text-[9px] uppercase text-muted-foreground">
            ...
            {accepted.length === 0 && (
              <p className="py-4 text-sm text-muted-foreground">
                Todavía no tienes amigos. Comparte tu código para empezar.
              </p>
            )}
          </div>
        </section>
        ...
        {pendingOutgoing.length > 0 && (
          <section className="journal-in">
            <p className="eyebrow">Enviadas, pendientes de respuesta</p>
            <p className="mt-2 font-mono text-[11px] text-muted-foreground">
              {pendingOutgoing.map((l) => l.other?.username).join(", ")}
            </p>
          </section>
        )}
      </div>
